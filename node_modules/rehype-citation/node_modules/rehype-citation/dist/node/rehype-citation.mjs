...
import fetch$4 from 'cross-fetch';
import parse5 from 'parse5';
import { fromParse5 } from 'hast-util-from-parse5';
import * as name$1 from '@citation-js/name';
import { parse as parse$7, format as format$7 } from '@citation-js/name';
...
import { parse as parse$8, format as format$8 } from '@citation-js/date';
import CSL$1 from 'citeproc';
...
const readFile = async path => {
  if (isValidHttpUrl(path)) {
    return fetch$4(path).then(response => response.text()).then(data => data);
  } else {
    {
...
const rehypeCitationGenerator = Cite => {
  return (options = {}) => {
    return async (tree, file) => {
      ...
      if (isValidHttpUrl(bibliography)) {
        const response = await fetch$4(bibliography);
...
const retrieveVersion = function retrieveVersion(versnum = 1) {
  if (versnum <= 0 || versnum > this.currentVersion()) {
    ...
    const image = new Cite$1(JSON.parse(data), JSON.parse(options));
...
const options = function options(options, log) {
...
class TypeParser {
  constructor(data) {
    _defineProperty$5(this, 'validDataTypes', ['String', 'Array', 'SimpleObject', 'ComplexObject', 'Primitive']);
...
const correctName = function correctName(name, bestGuessConversions) {
...
const correctNameList = function correctNameList(nameList, bestGuessConversions) {
...
const correctDateParts = function correctDateParts(dateParts, bestGuessConversions) {
...
const correctDate = function correctDate(date, bestGuessConversions) {
  const dp = 'date-parts';
  ...
  if (typeof date !== 'object' || date === null) {
    ...
    return range.length ? _objectSpread$4(_objectSpread$4({}, date), {}, {
      'date-parts': range
    }) : undefined;
...
const correctType = function correctType(type, bestGuessConversions) {
...
const correctField = function correctField(fieldName, value, bestGuessConversions) {
...
const parseCsl = function parseCsl(data, bestGuessConversions = true) {
...
const data = (input, type) => {
  if (typeof parsers$2[type] === 'function') {
    return parsers$2[type](input);
...
const addDataParser = (format, {
  ...
  if (async) {
    ...
    parsers$2[format] = parser;
  }
};
const hasDataParser = (type, async) => async ? asyncParsers[type] || nativeAsyncParsers[type] : parsers$2[type] || nativeParsers[type];
const removeDataParser = (type, async) => {
  delete (async ? asyncParsers : parsers$2)[type];
};
const listDataParser = async => Object.keys(async ? asyncParsers : parsers$2);

const formats$3 = {};
const add$5 = (format, parsers) => {
...
const has$4 = format => format in formats$3;
const list$4 = () => Object.keys(formats$3);
...
const add$4 = function add(data, options = {}, log = false) {
...
const addAsync = async function addAsync(data, options = {}, log = false) {
  if (options === true || log === true) {
    this.save();
  }
...
const setAsync = async function setAsync(data, options = {}, log = false) {
  if (options === true || log === true) {
    this.save();
  }
...
const getComparisonValue = function getComparisonValue(obj, prop, label = prop === 'label') {
  ...
  switch (prop) {
    case 'author':
    case 'editor':
      return value.map(name => name.literal || name.family || format$7(name));
...
const compareProp = function compareProp(entryA, entryB, prop, flip = /^!/.test(prop)) {
  prop = prop.replace(/^!/, '');
...
const async = function async(data, options, callback) {
  ...
  const promise = Cite$1().setAsync(data, options);
...
const validate$2 = (name, formatter) => {
...
const format$6 = (name, data, ...options) => {
...
function format$5(format, ...options) {
  return format$6(format, parseCsl(this.data), ...options);
}
...
Object.assign(Cite$1.prototype, log, options$1, set$1, sort$1, get$3); // No need for generator as we are not using it
...
const configs = {};
const add$1 = (ref, config) => {
  configs[ref] = config;
};
const get = ref => configs[ref];
const has$1 = ref => Object.prototype.hasOwnProperty.call(configs, ref);
const remove$1 = ref => {
  delete configs[ref];
};
const list$1 = () => Object.keys(configs);
...
const INTERNALS$3 = Symbol('Body internals'); // fix an issue where "PassThrough" isn't a named export for node <10
...
function exportNodeCompatibleHeaders$1(headers) {
  const obj = Object.assign({
    __proto__: null
  }, headers[MAP$1]); // http.request() only supports string as Host header. This hack makes
...
const INTERNALS$1$1 = Symbol('Response internals'); // fix an issue where "STATUS_CODES" aren't a named export for node <10
...
const URL$3 = Url.URL || publicApi.URL; // fix an issue where "format", "parse" aren't a named export for node <10
...
function getNodeRequestOptions$1(request) {
  ...
  const headers = new Headers$2(request[INTERNALS$2$1].headers); // fetch step 1.3
...
const URL$1$2 = Url.URL || publicApi.URL; // fix an issue where "PassThrough", "resolve" aren't a named export for node <10
...
function fetch$3(url, opts) {
  ...
  Body$2.Promise = fetch$3.Promise; // wrap http.request into fetch
  ...
  return new fetch$3.Promise(function (resolve, reject) {
    ...
    req.on('response', function (res) {
      clearTimeout(reqTimeout);
      const headers = createHeadersLenient$1(res.headers); // HTTP fetch step 5
...
function serializeRequest(request) {
  return [request.url, {
    ...
    timeout: request.timeout,
    compress: request.compress,
    size: request.size // agent: request.agent

  }];
}

function deserializeRequest(fetch, resource, init) {
  return new fetch.Request(resource, { ...init,
...
function deserializeResponse(fetch, body, init, bodyError) {
  ...
  return new fetch.Response(body ? createStream(buffer) : createStream(null), { ...init,
    headers: deserializeHeaders(fetch, init.headers)
  }, {
    buffer,
    bodyError
...
function fetch$2(resource, init) {
  ...
  request[1].headers = shared.serializeHeaders(request[1].headers); // TODO credentials
...
const URL$1 = Url.URL || publicApi.URL; // fix an issue where "format", "parse" aren't a named export for node <10
...
function getNodeRequestOptions(request) {
  ...
  const headers = new Headers$1(request[INTERNALS$2].headers); // fetch step 1.3
...
const URL$1$1 = Url.URL || publicApi.URL; // fix an issue where "PassThrough", "resolve" aren't a named export for node <10
...
createCommonjsModule(function (module) {
  ...
  module.exports = function (url, options) {
    ...
    return lib.call(this, url, options);
  };

  if (!commonjsGlobal.fetch) {
    commonjsGlobal.fetch = module.exports;
    commonjsGlobal.Response = lib.Response;
    commonjsGlobal.Headers = lib.Headers;
    commonjsGlobal.Request = lib.Request;
  }
});
...
var coreOutput = Object.assign({}, json$1, label);
...
function generalProps(input) {
  ...
  if (input.date && Object.keys(input.date).length > 0) {
    ...
    if (dates.submitted) {
      output.submitted = parse$8(dates.submitted);
    }

    if (dates.published) {
      output.issued = parse$8(dates.published);
    }
  } else if (input.year) {
    output.issued = {
...
const lexer$1 = moo.states({
  ...
  entry: _objectSpread$3(_objectSpread$3({}, whitespace), {}, {
    otherEntryType: {
  ...
  otherEntryContents: _objectSpread$3(_objectSpread$3({}, whitespace), {}, {
    lbrace: {
      match: /[{(]/,
      next: 'fields'
    }
  }),
  dataEntryContents: _objectSpread$3(_objectSpread$3({}, whitespace), {}, {
    lbrace: {
      match: /[{(]/,
      next: 'dataEntryContents'
    },
  ...
  fields: _objectSpread$3(_objectSpread$3({}, whitespace), {}, {
    identifier,
    number: /-?\d+/,
    hash: '#',
    equals: '=',
...
const Converters = {
  ...
  LABEL: {
    ...
    toSource(id, label, author, issued, suffix, title) {
      ...
      if (config$1.format.useIdAsLabel) {
        return safeId;
      }
...
const nonSpec = [{
  ...
  convert: {
    toSource(accessed) {
      return `[Online; accessed ${format$8(accessed)}]`;
    }
...
var biblatex = new Translator([...aliases, ...nonSpec, {
  ...
  convert: {
    toTarget(type, subtype, typeKey) {
      ...
      return [biblatexTypes.source[type] || 'book', typeKey || subtype];
    },
...
var bibtex = new Translator([{
  ...
  convert: {
    toTarget(sourceType, subType) {
      const type = bibtexTypes.source[sourceType] || 'book';
...
function getMainRule(fieldType, languages) {
  ...
  if (fieldType[1] === 'title') {
    const option = config$1.parse.sentenceCase;
...
function parse$1(text, field, languages = []) {
...
function parseEntryValues(entry) {
  ...
  if ('language' in entry.properties) {
    output.language = parse$1(entry.properties.language, 'language');
  }
  ...
  for (const property in entry.properties) {
    ...
    output[property] = parse$1(value + '', property, output.language);
  }
...
function parse(entries) {
  if (config$1.parse.strict) {
    validate(entries, required.biblatex);
  }

  return parse$2(entries.map(parseEntryValues));
}
...
function formatEntryValues({
  ...
  for (const property in properties) {
    ...
    output[property] = format$3(property, value);
  }
...
function format$2(entries) {
  return format$4(entries).map(formatEntryValues);
}
function formatBibtex(entries) {
  return formatBibtex$1(entries).map(formatEntryValues);
}

var bibtexOutputEntries = {
  __proto__: null,
  format: format$2,
...
var bibtexOutput = {
  bibtex: factory(formatBibtex, format$1),
  biblatex: factory(format$2, format$1),
  bibtxt: factory(formatBibtex, format)
};
...
for (const format in CSL$1.Output.Formats) {
  const original = CSL$1.Output.Formats[format]['@bibliography/entry'];

  if (!original || original[proxied]) {
    continue;
  }

  CSL$1.Output.Formats[format]['@bibliography/entry'] = getWrapperProxy(original);
}
...
const fetchEngine = function fetchEngine(style, lang, template, retrieveItem, retrieveLocale) {
  ...
  if (engines[engineHash] instanceof CSL$1.Engine) {
    engine = engines[engineHash];
    engine.sys.retrieveItem = retrieveItem;
    engine.updateItems([]);
  } else {
    engine = engines[engineHash] = new CSL$1.Engine({
      retrieveLocale,
      retrieveItem
    }, template, lang, true);
  }
...
const prepareEngine = function prepareEngine(data, templateName, language, format) {
  if (!CSL$1.Output.Formats[format] || !CSL$1.Output.Formats[format]['@bibliography/entry']) {
...
Cite.prototype = Object.create(Cite$1.prototype);
Cite.async = Cite$1.async;
Cite.validateOptions = Cite$1.validateOptions;
Cite.validateOutputOptions = Cite$1.validateOutputOptions;
Cite.input = chain;
Cite.inputAsync = chainAsync;
Cite.util = Object.assign({
  attr
}, index);
Cite.version = {
  cite: version,
  citeproc: CSL$1.PROCESSOR_VERSION
};
const CSL = get('@csl');
Cite.CSL = {
  engine: fetchEngine,
...
Cite.parse = Object.assign({
  ...
  json: parsers$1.json.parse // wikidata: ((wikidata) => ({
...
Cite.get = Object.assign({
  ...
  bibtex: ((formatters, entries, converters, types) => ({
    ...
    text(entries, asHtml) {
      return formatters.bibtex(entries, {
...
export { Cite, rehypeCitation as default, rehypeCitationGenerator };
