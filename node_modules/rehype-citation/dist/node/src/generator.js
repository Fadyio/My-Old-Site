 ...
 * @return {*}
 */
const genCitation = (citeproc, entries, citationId, citationPre, options, properties = { noteIndex: 0 }) => {
    ...
    const ids = `citation--${entries.map((x) => x.id.toLowerCase()).join('--')}--${citationId.split('-')[1]}`;
    // Coerce to html to parse HTML code e.g. &#38; and return text node
    return htmlToHast(`<span class="${(options.inlineClass ?? []).join(' ')}" id=${ids}>${result[1]}</span>`);
};
/**
    ...
    return (options = {}) => {
        return async (tree, file) => {
            ...
            visit(tree, 'text', (node, idx, parent) => {
                ...
                const citedTextNode = genCitation(citeproc, entries, `CITATION-${citationId}`, citationPre, options, properties);
